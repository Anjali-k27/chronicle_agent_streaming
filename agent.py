...
def make_pattern_node(utility_llm: "ChatGoogleGenerativeAI", mcp_pool: "MCPClientPool"):
    ...
    async def pattern_node(state: ChronicleState) -> dict:
        ...
        response = await utility_llm.ainvoke([HumanMessage(content=prompt)])
        correlations = [
            line.strip()
        ...
        if "spotify" in state["data_sources"]:
            ...
            result = await mcp_pool.fetch_source("spotify", {"days": 30})
            tool_results.append({
                "name":    "get_spotify_history",
                "status":  "success" if result["live"] else "fallback",
                "summary": f"{result['count']} tracks retrieved ({'live' if result['live'] else 'calibration data'})",
            })
...
def make_brutality_node(frontier_llm: "ChatGoogleGenerativeAI", mcp_pool: "MCPClientPool"):
    ...
    async def brutality_node(state: ChronicleState) -> dict:
        ...
        response = await frontier_llm.ainvoke([HumanMessage(content=prompt)])
        elapsed_ms = round((time.monotonic() - start) * 1000)
        return {
            "honest_analysis": response.content,
...
# ── CLI Entry Point (Session 12.2 — UPDATED) ─────────────────────

if __name__ == "__main__":
    print("\n╔══════════════════════════════════════════════════════╗")
    print("║  Chronicle — Session 12.2 Verification               ║")
    print("╚══════════════════════════════════════════════════════╝\n")
    ...
    if result["passed"]:
        print("  ✓ Session 12.2 COMPLETE. Start the API: python api.py")
        print("  Test the stream: curl -N -X POST http://localhost:8000/analyze/stream \\")
...
# ══════════════════════════════════════════════════════════════════
# SESSION 12.3 HANDOFF — "Resilience & Queuing"
# ══════════════════════════════════════════════════════════════════
#
# What gets ADDED in Session 12.3 (extend, never remove):
#   JobStatus enum: queued, processing, completed, failed, cancelled
#   JobRecord Pydantic model: job_id, ticket_id, status, active_node,
#     partial_result, final_result, error_message, heartbeat timestamps
#   JobAcceptedResponse: 202 response body with job_id + poll_url
#   write_job() / read_job() / update_status(): async job store ops
#   run_chronicle_analysis(): background worker wrapping graph.ainvoke()
#     writes status at every Chronicle node transition
#
# What stays UNCHANGED from Session 12.2:
#   NODE_LABELS, AGENT_NODES
#   chronicle_stream_events() — used by /analyze/stream (preserved)
#   tool_calls / tool_results accumulation pattern
#   ChronicleState TypedDict fields
#   build_chronicle_graph() topology
#   stream_schemas.py event types (public API contract)
#   All S11.1 / S11.2 constants and functions
# ══════════════════════════════════════════════════════════════════
